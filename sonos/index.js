...
const { IntercomBridge } = require('./intercom-bridge');
...
class SonosSystem extends EventEmitter {
  constructor({ port } = {}) {
    ...
    // Adapter for the Intercom feature — resolves a room name to a player and
    // exposes raw capture/restore/broadcast primitives so the Intercom flow can
    // bypass group-coordinator routing for the duration of a message.
    this.intercom = new IntercomBridge(this.topology);
